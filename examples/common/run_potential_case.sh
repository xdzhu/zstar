...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help) echo 'Usage: run_potential_case.sh --case-dir DIR --mode mos2|in2se3|ges|directional [--cube FILE] [--dry-run]'; exit 0 ;;
...
case "$mode" in
    ...
    ges)
        ...
        compgen -G "$asset_root/*.orb" >/dev/null || { echo "Missing orbital files in $asset_root" >&2; exit 3; }
        tile=(3 3)
        extra=(--direction a --direction b --direction a+b --direction-bins 160 --direction-method linear --direction-samples 72 72 --direction-smooth 0.15 --mirror-test) ;;
    directional)
    ...
    *) echo '--mode must be mos2, in2se3, ges, or directional' >&2; exit 2 ;;
esac
...
2. zstar pot --cube "$cube" --axes z --plane xy --plane-average --tile ${tile[*]} \
...
zstar pot --cube "$cube" --axes z --plane xy --plane-average --tile "${tile[@]}" \
    --vacuum-level --vacuum-sides --vacuum-window 0.75 \
    "${extra[@]}" --outdir "$case_dir/work/potential"
