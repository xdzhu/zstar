...
当前源码通过全部 37 项测试。被忽略的本地 BTO 算例也成功执行
...
hBN 选定模式和 MoS2 的完整光学模式集合用于检查二维片层极化率约定；四方
BTO 则扩展到全部 10 个正频光学模式，共完成 20 个正负法向坐标电子响应任务：
...
- [已验证的分子、MoS2 与 Bulk IR/Raman 对比图](paper_figures/spectroscopy_validated_dimensions.png)
- [保留 1D 行的 0D--3D 光谱路线图](paper_figures/spectroscopy_across_dimensions.png)
