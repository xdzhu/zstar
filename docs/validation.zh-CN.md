...
- 调度脚本：在直连计算环境检查 shell 与 Torque/PBS，在 Slurm 集群检查
  Slurm 脚本和运行环境。
...
发布候选版本通过全部 32 项源码测试。被忽略的本地 BTO 算例也成功执行
...
因此不再采用。无法使用静态直算时，ZStar 0.1.0 默认使用经过验证的
0-30 eV 紧凑区间。
...
这些选定模式用于验证数据传递、归一化、中心差分、张量收集和光谱输出，
不代表已经对实验峰位或强度完成生产级收敛。
